...
int main() {
	...
	char a[100], b[100], * tmp1, * tmp2, ** words_A, ** words_B, ** words_AB, temp[101];
	...
	for (int i = 0; i < space_A; i++) {
		for (int j = 0; j < space_B; j++) {
			...
			for (int p = 0; p < space_A; p++) {
				strcpy(temp, words_A[i]);

				if (p != i) strcpy(temp2, strcat(words_A[i], words_A[p])); // 문자열 결합
				else strcpy(temp2, "0");

				// 합성된 문자열 갯수 카운트
				if (strcmp(temp2, words_B[j]) == 0) space_AB++;

				strcpy(words_A[i], temp); // 문자열 초기화
	...
	for (int i = 0; i < space_A; i++) {
		...
		for (int j = 0; j < space_B; j++) {
			...
			for (int p = 0; p < space_A; p++) {
				...
				// 합성된 문자열 존재시 저장
				if (strcmp(temp2, words_B[j]) == 0) {
					words_AB[k] = (char*)malloc((strlen(temp2)) * sizeof(char));
					if (words_AB[k] == NULL) return -1;

					strcpy(words_AB[k], temp2);
					words_AB[k][strlen(temp2)] = NULL;
					k++;
				}
	...
	for (int i = 0; i < space_AB; i++) {
		...
		else if (strcmp(words_AB[i], words_AB[i - 1]) != 0) puts(words_AB[i]);
